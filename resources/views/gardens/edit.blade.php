    ...
    <div class="py-8">
        <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <div class="mt-2 bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <x-card title="Edit Garden" :subtitle="'Garden #' . $garden->id">
                    @if ($errors->any())
                    ...
                    <form action="{{ route('gardens.update', $garden) }}" method="POST" class="space-y-4">
                        @csrf
                        @method('PATCH')
                        ...
                        <div>
                            ...
                            <input id="name" name="name" type="text" value="{{ old('name', $garden->name) }}"
                                required
                        ...
                        <div>
                            ...
                            <input id="address" name="address" type="text"
                                value="{{ old('address', $garden->address) }}" required
                                class="w-full rounded-md border border-gray-300 px-3 py-2 text-gray-900 focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500 dark:border-gray-600 dark:bg-gray-900 dark:text-gray-100" />
                        </div>

                        <div class="mt-2 flex gap-2">
                            <a href="{{ route('gardens.index', $garden) }}"
